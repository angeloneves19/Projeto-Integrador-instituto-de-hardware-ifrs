...
import { Loader2, User, Calendar, Clock, Check, X, MessageSquare } from 'lucide-react'
...
export default function PainelContatos() {
  ...
  async function aceitar(contato) {
    ...
    // Sem data desejada, não dá pra checar conflito de agenda — aceita direto.
    if (!contato.data_desejada || !contato.turno) {
    ...
    const horario = TURNO_HORARIO[contato.turno] || TURNO_HORARIO.manha
    const dataInicio = `${contato.data_desejada}T${horario.inicio}:00`
    const dataFim = `${contato.data_desejada}T${horario.fim}:00`

    // Checa se já existe algo marcado nesse mesmo período pra essa profissional.
    ...
    if (conflitos?.length > 0) {
      setAvisoConflito(`Você já tem um compromisso em ${contato.data_desejada} no turno da ${TURNO_LABEL[contato.turno]}. Recuse ou combine outro horário com o cliente.`)
      setProcessando(null)
      return
    }
  ...
  const outros = contatos.filter((c) => c.status !== 'pendente')
